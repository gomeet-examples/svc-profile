...
import (
	...
	gomeetfaker_plugin "github.com/gomeet/go-proto-gomeetfaker/plugin"
)
...
func main() {
	...
	gen.GeneratePlugin(gomeetfaker_plugin.NewPlugin(useGogoImport))

	for i := 0; i < len(gen.Response.File); i++ {
		gen.Response.File[i].Name = proto.String(strings.Replace(*gen.Response.File[i].Name, ".pb.go", ".gomeetfaker.pb.go", -1))
	}
