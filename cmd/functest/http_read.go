...
func TestHttpRead(config FunctionalTestConfig) (failures []TestFailure) {
	...
	if err != nil {
		failures = append(failures, TestFailure{Procedure: "Read/HTTP", Message: fmt.Sprintf("HTTP client initialization error (%v)", err)})
		return failures
	}
	...
	if err != nil {
		failures = append(failures, TestFailure{Procedure: "Read/HTTP", Message: fmt.Sprintf("HTTP testGetReadRequest error (%v)", err)})
		return failures
	}
		...
		if err != nil {
			testCaseResults = append(
				...
				&TestCaseResult{
					req,
					nil,
					fmt.Errorf("Read/HTTP POST error on %s (%v) - %v - readAll body", url, err, req),
				},
			)
			continue
		}
		...
		if err == nil && (httpError.Code != 0 || httpError.Error != "") {
			testCaseResults = append(
				...
				&TestCaseResult{
					req,
					nil,
					fmt.Errorf("Read/HTTP POST error on %s (Code: %d, Error: %s) - %v", url, httpError.Code, httpError.Error, req),
				},
			)
			continue
		}
	...
	return testReadResponse(config, FUNCTEST_HTTP, testCaseResults, extras)
}
