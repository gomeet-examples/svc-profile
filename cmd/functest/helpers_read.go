...
func testGetReadRequest() (reqs []*pb.ProfileRequest) {
	// return an array of pb.ProfileRequest struct pointers,
	// each of them will be passed as an argument to the grpc Read method
...
func testReadResponse(
	...
	for _, tr := range testCaseResults {
		...
		if tr.Response != nil {
			...
			if !ok {
				failures = append(failures, TestFailure{Procedure: "Read", Message: "expected response message type pb.ProfileRequest - cast fail"})
				continue
			}
		}
