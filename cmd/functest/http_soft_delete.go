		...
		if err != nil {
			testCaseResults = append(
				...
				&TestCaseResult{
					req,
					nil,
					fmt.Errorf("SoftDelete/HTTP POST error to marshalling the message with %s (%v) - %v", url, err, req),
				},
			)
			continue
		}
		...
		if err != nil {
			testCaseResults = append(
				...
				&TestCaseResult{
					req,
					nil,
					fmt.Errorf("SoftDelete/HTTP POST error to construct the http request with %s (%v) - %v", url, err, req),
				},
			)
			continue
		}
		...
		if err != nil {
			testCaseResults = append(
				...
				&TestCaseResult{
					req,
					nil,
					fmt.Errorf("SoftDelete/HTTP POST error on %s (%v) - %v", url, err, req),
				},
			)
			continue
		}
