...
func testGetHardDeleteRequest() (reqs []*pb.ProfileRequest) {
	// return an array of pb.ProfileRequest struct pointers,
	// each of them will be passed as an argument to the grpc HardDelete method
...
func testHardDeleteResponse(
	...
	for _, tr := range testCaseResults {
		...
		if tr.Response != nil {
			...
			if !ok {
				failures = append(failures, TestFailure{Procedure: "HardDelete", Message: "expected response message type pb.ProfileRequest - cast fail"})
				continue
			}
		}
